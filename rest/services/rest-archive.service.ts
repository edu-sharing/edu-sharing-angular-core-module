...
export class RestArchiveService extends AbstractRestService {
    ...
    public search = (
        ...
        let query = this.connector.createUrlNoEscape(
            'archive/:version/search/:repository/:pattern/:person?:request',
            repository,
            [
    ...
    public delete = (
        ...
        let query = this.connector.createUrlNoEscape(
            'archive/:version/purge/:repository/?:nodes',
            repository,
            [[':nodes', RestHelper.getQueryStringForList('archivedNodeIds', nodes)]],
        );

        return this.connector
            .delete(query, this.connector.getRequestOptions())
    ...
    public restore = (
        ...
        let query = this.connector.createUrlNoEscape(
            'archive/:version/restore/:repository/?:nodes&target=:target',
            repository,
            [
                [':nodes', RestHelper.getQueryStringForList('archivedNodeIds', nodes)],
