...
export class RestMediacenterService extends AbstractRestService {
  ...
  public getMediacenters(repository = RestConstants.HOME_REPOSITORY) {
    const query = this.connector.createUrl('mediacenter/:version/mediacenter/:repository', repository);
  ...
  public addMediacenter(mediacenter:string,profile:any,repository = RestConstants.HOME_REPOSITORY) {
    const query = this.connector.createUrl('mediacenter/:version/mediacenter/:repository/:mediacenter', repository,[
      [':mediacenter',mediacenter]
    ]);
    return this.connector.post<Mediacenter>(query, JSON.stringify(profile), this.connector.getRequestOptions());
  }
  public editMediacenter(mediacenter:string,profile: MediacenterProfile,repository = RestConstants.HOME_REPOSITORY) {
    const query = this.connector.createUrl('mediacenter/:version/mediacenter/:repository/:mediacenter', repository,[
        [':mediacenter',mediacenter]
    ]);
  ...
  public addManagedGroup(mediacenter:string,group:string,repository = RestConstants.HOME_REPOSITORY) {
    const query = this.connector.createUrl('mediacenter/:version/mediacenter/:repository/:mediacenter/manages/:group', repository,[
      [':mediacenter',mediacenter],
      [':group',group]
    ]);
  ...
  public getManagedGroups(mediacenter:string,repository = RestConstants.HOME_REPOSITORY) {
    const query = this.connector.createUrl('mediacenter/:version/mediacenter/:repository/:mediacenter/manages', repository,[
      [':mediacenter',mediacenter]
    ]);
  ...
  public getLicensedNodes(mediacenter:string, searchWord:string, repository = RestConstants.HOME_REPOSITORY,
                          facettes : string[] = [],
                          request : any = null) {
      console.log(searchWord);
        const query=this.connector.createUrlNoEscape('mediacenter/:version/mediacenter/:repository/:mediacenter/licenses?:searchword&:facettes&:request',repository,
            [
                ...
                [':searchword',RestHelper.getQueryString('searchword', [searchWord])],
