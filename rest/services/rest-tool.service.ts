...
export class RestToolService extends AbstractRestService {
    ...
    public createToolDefinition = (
        ...
        let query = this.connector.createUrlNoEscape(
            'tool/:version/tools/:repository/tooldefinitions/?renameIfExists=:rename&versionComment=:versionComment',
            repository,
            [
    ...
    public createToolInstance = (
        ...
        let query = this.connector.createUrlNoEscape(
            'tool/:version/tools/:repository/:parent/toolinstances/?renameIfExists=:rename&versionComment=:versionComment',
            repository,
            [
     ...
     * @returns {Observable<R>}
     */
    public getToolInstances = (
        tooldefinition: string,
        repository = RestConstants.HOME_REPOSITORY,
    ): Observable<NodeList> => {
        let query = this.connector.createUrl(
            'tool/:version/tools/:repository/:tooldefinition/toolinstances',
            repository,
    ...
    public openLtiObject(node: Node, win: Window = null) {
        let req =
            ...
            '../eduservlet/connector?nodeId=' +
            encodeURIComponent(node.ref.id);
        if (win == null) win = window.open('', '_blank');
