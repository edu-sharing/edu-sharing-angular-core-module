...
export class RestNodeService extends AbstractRestService{
  ...
  public updateNodeShare = (node : string,
    ...
    if (typeof password === 'string') {
      query=this.connector.createUrl("node/:version/nodes/:repository/:node/shares/:shareId?expiryDate=:expiryDate&password=:password",repository,
        [
          [":node",node],
          [":shareId",shareId],
          [":expiryDate",""+expiryDate],
      ...
      query=this.connector.createUrl("node/:version/nodes/:repository/:node/shares/:shareId?expiryDate=:expiryDate",repository,
        [
          [":node",node],
          [":shareId",shareId],
          [":expiryDate",""+expiryDate],
  ...
  public editNodeMetadataNewVersion = (node : string,versionComment:string,properties : any,repository=RestConstants.HOME_REPOSITORY) => {
    ...
    public editNodeProperty = (node : string,property: string, value: string,repository=RestConstants.HOME_REPOSITORY) => {
        let query = this.connector.createUrl("node/v1/nodes/:repository/:node/property?property=:property&value=:value", repository,
            [
                [":node", node],
                [":property", property],
                [":value", value],
