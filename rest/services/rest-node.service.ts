...
export class RestNodeService extends AbstractRestService {
     ...
     *
     */
    public getPublishedCopies = (node: string, repository = RestConstants.HOME_REPOSITORY) => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/publish',
            repository,
            [[':node', node]],
        );
    ...
    public publishCopy = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/publish?handleMode=:handleMode',
            repository,
            [
                [':node', node],
    ...
    public searchNodes = (
        ...
        let query = this.connector.createUrlNoEscape(
            'node/:version/nodes/:repository?query=:query&:facets&:request',
            repository,
            [
     ...
     * @returns {Observable<R>}
     */
    public prepareUsage = (node: string, repository = RestConstants.HOME_REPOSITORY) => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/prepareUsage',
            repository,
            [[':node', node]],
        );
    ...
    public getChildren = (
        ...
        let query = this.connector.createUrlNoEscape(
            'node/:version/nodes/:repository/:parent/children/?:filter&assocName=:assocName&:request',
            repository,
            [
    ...
    public reportNode = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/report?reason=:reason&userEmail=:userEmail&userComment=:userComment',
            repository,
            [
                [':node', node],
                [':reason', reason],
     ...
     */
    public importNode = (repository: string, node: string, localParent: string) => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/import/?parent=:parent',
            repository,
            [
                [':node', node],
                [':parent', localParent],
    ...
    public createNode = (
        ...
        let query = this.connector.createUrlNoEscape(
            'node/:version/nodes/:repository/:parent/children/?type=:type&renameIfExists=:rename&assocType=:assocType&versionComment=:versionComment&:aspects',
            repository,
            [
                [':parent', encodeURIComponent(parent)],
    ...
    public copyNode = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:target/children/_copy?source=:toCopy&withChildren=:recursive',
            repository,
            [
                [':target', target],
                [':toCopy', toCopy],
    ...
    public moveNode = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:target/children/_move?source=:toCopy',
            repository,
            [
                [':target', target],
                [':toCopy', toMove],
    ...
    public forkNode = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:target/children/_fork?source=:toFork',
            repository,
            [
                [':target', target],
                [':toFork', toFork],
    ...
    public getNodeShares = (
        node: string,
        email: string = null,
        repository = RestConstants.HOME_REPOSITORY,
    ) => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/shares?email=:email',
            repository,
            [
                [':node', node],
                [':email', email],
    ...
    public createNodeShare = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/shares/?expiryDate=:expiryDate&password=:password',
            repository,
            [
                [':node', node],
    ...
    public deleteNodeShare = (
        node: string,
        share: string,
        repository = RestConstants.HOME_REPOSITORY,
    ): Observable<Response> => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/shares/:share',
            repository,
            [
                [':node', node],
                [':share', share],
    ...
    public getNodeMetadata = (
        ...
        let query = this.connector.createUrlNoEscape(
            'node/:version/nodes/:repository/:node/metadata/?:propertyFilter',
            repository,
            [
     ...
     * @returns {Observable<R>}
     */
    public getNodeParents = (
        node: string,
        fullPath = this.connector.getCurrentLogin()
            ? this.connector.getCurrentLogin().isAdmin
            : false,
        //fullPath=false,
        propertyFilter: string[] = [],
        repository = RestConstants.HOME_REPOSITORY,
    ): Observable<ParentList> => {
        let query = this.connector.createUrlNoEscape(
            'node/:version/nodes/:repository/:node/parents/?fullPath=:fullPath&:propertyFilter',
            repository,
            [
                [':node', encodeURIComponent(node)],
    ...
    public deleteNode = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node?recycle=:recycle',
            repository,
            [
                [':node', node],
                [':recycle', '' + recycle],
     ...
     * @returns {Observable<R>}
     */
    public getNodeMetadataForVersion = (
        node: string,
        major: number,
        minor: number,
        repository = RestConstants.HOME_REPOSITORY,
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/versions/:major/:minor/metadata',
            repository,
            [
                [':node', node],
                [':major', major + ''],
    ...
    public getNodePermissions = (node: string, repository = RestConstants.HOME_REPOSITORY) => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/permissions',
            repository,
            [[':node', node]],
        );
     ...
     * @returns {Observable<R>}
     */
    public getNodePermissionsForUser = (
        node: string,
        authority: string,
        repository = RestConstants.HOME_REPOSITORY,
    ) => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/permissions/:authority',
            repository,
            [
                [':node', node],
                [':authority', authority],
    ...
    public getNodePermissionsHistory(node: string, repository = RestConstants.HOME_REPOSITORY) {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/notifys',
            repository,
            [[':node', node]],
        );
        return this.connector.get<NodePermissionsHistory[]>(
            query,
            this.connector.getRequestOptions(),
        );
    }
    ...
    public getNodeVersions = (node: string, repository = RestConstants.HOME_REPOSITORY) => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/versions',
            repository,
            [[':node', node]],
        );
    ...
    public revertNodeToVersion = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/versions/:major/:minor/_revert',
            repository,
            [
                [':node', node],
                [':major', major + ''],
    ...
    public setNodePermissions = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/permissions?mailtext=:mailText&sendMail=:sendMail&sendCopy=:sendCopy',
            repository,
            [
                [':node', node],
                [':mailText', mailText],
     ...
     * @returns {Observable<R>}
     */
    public editNodeMetadataNewVersion = (
        node: string,
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/metadata?versionComment=:comment',
            repository,
            [
                [':node', node],
     ...
     * @returns {Observable<R>}
     */
    public AddNodeAspects = (
        node: string,
        aspects: string[],
        repository = RestConstants.HOME_REPOSITORY,
    ) => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/aspects',
            repository,
            [[':node', encodeURIComponent(node)]],
        );
     ...
     * @returns {Observable<R>}
     */
    public editNodeMetadata = (
        node: string,
        properties: any,
        repository = RestConstants.HOME_REPOSITORY,
    ) => {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/metadata',
            repository,
            [[':node', node]],
        );
        return this.connector.put<NodeWrapper>(
            query,
            JSON.stringify(properties),
            this.connector.getRequestOptions(),
        );
    ...
    public setNodeTextContent = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/content?versionComment=:comment&mimetype=:mimetype',
            repository,
            [
                [':node', node],
                [':mimetype', mimetype],
    ...
    public addWorkflow = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/workflow',
            repository,
            [[':node', node]],
        );
        return this.connector.put(
    ...
    public isLocked = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/lock/status',
            repository,
            [[':node', node]],
        );
    ...
    public deleteNodePreview = (
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/preview',
            repository,
            [[':node', node]],
        );
        let options = this.connector.getRequestOptions();
    ...
    private createNodeUrl(data: any) {
        ...
        this.createNode(
            ...
            (data: NodeWrapper) => {
                this.bridge.showTemporaryMessage(MessageType.info, 'NODE_CREATED_USERHOME', {
                    name: data.node.name,
                });
    ...
    public getNodeTextContent(node: string, repository = RestConstants.HOME_REPOSITORY) {
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/textContent',
            repository,
            [[':node', node]],
        );
    ...
    public setNodeTemplate(
        ...
        let query = this.connector.createUrl(
            'node/:version/nodes/:repository/:node/metadata/template?enable=:enable',
            repository,
            [
                [':node', node],
                [':enable', '' + enable],
        ...
        return this.connector.put<NodeTemplate>(
            query,
            JSON.stringify(properties),
            this.connector.getRequestOptions(),
        );
    ...
    public getNodeChildobjects(nodeId: string, repository = RestConstants.HOME_REPOSITORY) {
        return this.getChildren(
            ...
            [],
            {
                ...
                sortBy: [RestConstants.CCM_PROP_CHILDOBJECT_ORDER],
                sortAscending: [true],
            },
