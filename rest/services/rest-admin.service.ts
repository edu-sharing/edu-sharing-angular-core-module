...
export class RestAdminService extends AbstractRestService {
    ...
    public importOAI(
        ...
        let query = this.connector.createUrl(
            'admin/:version/import/oai?baseUrl=:baseUrl&set=:set&metadataPrefix=:metadataPrefix&className=:className&importerClassName=:importerClassName' +
                '&recordHandlerClassName=:recordHandlerClassName&binaryHandlerClassName=:binaryHandlerClassName&persistentHandlerClassName=:persistentHandlerClassName&metadataset=:metadataset&fileUrl=:fileUrl&oaiIds=:ids&forceUpdate=:forceUpdate&from=:from&until=:until&periodInDays=:periodInDays',
            null,
            [
                [':baseUrl', baseUrl],
                [':set', set],
    ...
    public refreshCache(rootFolder: string, sticky = false) {
        let query = this.connector.createUrl(
            'admin/:version/import/refreshCache/:rootFolder?sticky=:sticky',
            null,
            [
    ...
    public getCacheInfo(id: string) {
        let query = this.connector.createUrl('admin/:version/cache/cacheInfo/:id', null, [
            [':id', id],
        ]);
    ...
    public getPropertyValuespace(property: string) {
        let query = this.connector.createUrl(
            'admin/:version/propertyToMds?properties=:property',
            null,
            [[':property', property]],
        );
        return this.connector.get<any>(query, this.connector.getRequestOptions());
    ...
    public runServerUpdate(id: string, execute = false) {
        let query = this.connector.createUrl(
            'admin/:version/serverUpdate/run/:id?execute=:execute',
            null,
            [
                [':id', id],
                [':execute', '' + execute],
    ...
    public searchLucene(lucene: string, store: string, authorities: string[], request: any = null) {
        let query = this.connector.createUrlNoEscape(
            'admin/:version/lucene?query=:lucene&:authorities&store=:store&:request',
            null,
            [
    ...
    public exportLucene(
        ...
        let query = this.connector.createUrlNoEscape(
            'admin/:version/lucene/export?query=:lucene&store=:store&:properties&:authorityScope',
            null,
            [
                [':lucene', encodeURIComponent(lucene)],
                [':store', encodeURIComponent(store)],
                ...
                [
                    ':authorityScope',
                    RestHelper.getQueryStringForList('authorityScope', authorities),
                ],
            ],
        );
        return this.connector.get<any>(query, this.connector.getRequestOptions());
    ...
    public startJobSync(job: string, params: any, file: File = null) {
        let query = this.connector.createUrl('admin/:version/job/:job/sync', null, [[':job', job]]);
        if (!params) {
            params = {};
        }
        if (file) {
            ...
            return this.connector.post<any>(
                query,
    ...
    public startJob(job: string, params: any, file: File = null) {
        let query = this.connector.createUrl('admin/:version/job/:job', null, [[':job', job]]);
        if (!params) {
            params = {};
        }
        if (file) {
    ...
    public removeDeletedImports(baseUrl: string, set: string, metadataPrefix: string) {
        let query = this.connector.createUrl(
            'admin/:version/import/oai/?baseUrl=:baseUrl&set=:set&metadataPrefix=:metadataPrefix',
            null,
            [
                [':baseUrl', baseUrl],
                [':set', set],
    ...
    public updateApplicationXML(xml: string, homeAppProperties: any[]) {
        ...
        return this.connector.put(
            query,
            JSON.stringify(homeAppProperties),
            this.connector.getRequestOptions(),
        );
    }
    ...
    public applyTemplate(groupName: string, templateName: string) {
        let query = this.connector.createUrl(
            'admin/:version/applyTemplate?template=:template&group=:group',
            null,
            [
    ...
    public deletePersons(user: string[], options: any) {
        let query = this.connector.createUrlNoEscape(
            'admin/:version/deletePersons?:username',
            null,
            [[':username', RestHelper.getQueryStringForList('username', user)]],
        );
        return this.connector.put<any>(
            query,
    ...
    public getConfigFile(filename: string, pathPrefix: ConfigFilePrefix) {
        let query = this.connector.createUrl(
            'admin/:version/configFile?filename=:filename&pathPrefix=:pathPrefix',
            null,
            [
                [':filename', filename],
